...
{
    public partial class ShipmentInfo2 : Form
    {
        ...
        private void loadEntry(string LotNbr)
        {
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    tbEntryOldWeightNetto.Text = dtEntry.Rows[0].ItemArray[19].ToString();
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
            ...
            if (!incrementError)
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    try
                    {
                        ...
                        using (SqlCommand command = new SqlCommand("Insert_ShipmentInfo_v3", connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@AcumaticaAdjNbr", adjs.Text);
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                try
                {
                    ...
                    foreach (SalesOrderDetail salesOrderDetail in salesOrderDetails)
                    {
                        ...
                        //Console.WriteLine("1:" +salesOrderDetail.Branch.Value.ToString());
                        //Console.WriteLine("2:" + salesOrderDetail.InventoryID.Value.ToString());
                        //Console.WriteLine("3:" + salesOrderDetail.Subitem.Value.ToString());
                        //Console.WriteLine("4:" + salesOrderDetail.OpenQty.Value.ToString());
                        //Console.WriteLine("5:" + salesOrderDetail.UOM.Value.ToString());
                        //Console.WriteLine("6:" + salesOrderDetail.LineNbr.Value.ToString());
                        //Console.WriteLine("7:" + salesOrderDetail.QtyOnShipments.Value.ToString());
                        //Console.WriteLine("8:" + salesOrderDetail.ShipOn.Value.ToString());
                        ...
                        using (SqlCommand command = new SqlCommand("Insert_ShipmentInfoDetail_C", connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@WarehouseID", salesOrderDetail.WarehouseID.Value);
                            ...
                            command.Parameters.AddWithValue("@DateShipment", salesOrderDetail.ShipOn.Value ?? null);
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    string query = $@"SELECT
                                    ...
                                    AND REPLACE(SubItem, '.', '') = '{subItemLot}' and InventoryID = '{inventoryLot}'";

                    Console.WriteLine(inventoryLot);
                    // qorik
        ...
        private void dgvDetail_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvDetail.SelectedRows.Count > 0)
            {
                ...
                inventoryLot = dgvDetail.Rows[dgvDetail.SelectedRows[0].Index].Cells[2].FormattedValue.ToString();
                loadComboLot();
            }
        }
        ...
        private void saveLot()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand("Insert_ShipmentInfoAllocation_C2", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@DocumentID", DocNumber);
                        ...
                        command.Parameters.AddWithValue("@OldWeightNetto", dtEntry.Rows[0].ItemArray[19].ToString());
                        command.ExecuteNonQuery();
                    }
                }
                catch (Exception e_update)
        ...
        private void dgvAlloc_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvAlloc.SelectedRows.Count > 0)
            {
                ...
                Ext_CropYear.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[25].Value.ToString();
                Ext_Quality.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[26].Value.ToString();
                Ext_HeadMark_GradeCustomer.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[27].Value.ToString();
                Ext_MarkingId.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[28].Value.ToString();
                Ext_Note.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[29].Value.ToString();
                tbEntryOldWeightNetto.Text = dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[30].Value.ToString().Trim()==""?dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[19].Value.ToString(): dgvAlloc.Rows[dgvAlloc.SelectedRows[0].Index].Cells[30].Value.ToString();
        ...
        private void btnAcumaticaShipment_Click(object sender, EventArgs e)
        {
            ...
            bool syncError = false;
            string referenceNbr = "";
            bool AdjustStatus=true;

            if (adjs.Text==""|| adjs.Text==null) {
                AdjustStatus = AdjustAcumatica();
            }

            if (AdjustStatus) {

                loadAllocation();
                DataView dv_filter = new DataView(dtAllocation, $"SyncDetail = 0", "InventoryID Asc , SubItem Asc , SOLine Asc", DataViewRowState.CurrentRows);
                ...
                foreach (DataRowView rowView in dv_filter)
                {
                    ...
                    //if (detail.Subitem.Value != curIndex&&detail.OrderLineNbr.Value)
                    if (detail.Subitem.Value != curIndex || detail.OrderLineNbr.Value != curIndex2)
                    {
                        indexarray++;
                        listDetail.Add(detail);
                        curIndex = detail.Subitem.Value;
                        curIndex2 = (int)detail.OrderLineNbr.Value;
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                try
                {
                    ...
                    string query = $@"SELECT
	                                        ...
	                                        ShipmentInfoAllocation.WeightNetto, 
 ...
 //               QRImage = QRImage,
        ...
        private void calweight() {
            tbEntryWeightNetto.Text = (Convert.ToDecimal(tbEntryWeightReceive.Text == "" ? "0" : tbEntryWeightReceive.Text.Replace(",", "")) - Convert.ToDecimal(tbEntryWeightTare.Text == "" ? "0" : tbEntryWeightTare.Text.Replace(",", ""))).ToString("N2");
        }
        ...
        private void TareChange(object sender, EventArgs e)
        {
            calweight();
        }

        public bool AdjustAcumatica() {

            bool AdjustStatus=true;
            
            if (dtDetail.Rows.Count > 0)
            {
                this.Text = $"Universal Leaf [{Warehouse.Descr}] - Marking Adjust [{DocNumber}] - Syncing with Acumatica, please wait!";
                bool syncError = false;
                bool allSynced = true;
                string docNbr = "";
                string referenceNbr = "";
                ...
                loadAllocation();
                DataView dv_filter = new DataView(dtAllocation, $"WeightNetto <> WeightShipping", "InventoryID Asc , SubItem Asc , SOLine Asc", DataViewRowState.CurrentRows);

                //issue
                Adjustment markingadjs = new Adjustment();
                markingadjs.Date = Convert.ToDateTime(tbShipmentDate.Text);
                markingadjs.Description = $"Weight Lose";
                markingadjs.ExternalRef = tbDocNumber.Text;
                markingadjs.Hold = false;

                List<AdjustmentDetail> markingadjsdetails = new List<AdjustmentDetail>();
                Console.WriteLine(dv_filter);

                foreach (DataRowView rowView in dv_filter)
                {
                    //check if lot not adjust

                    AdjustmentDetail markingadjsdetail = new AdjustmentDetail();
                    markingadjsdetail.InventoryID = rowView[1].ToString();
                    markingadjsdetail.BranchID = docBranch;
                    markingadjsdetail.LocationID = AcumaticaCred.AcumaticaInvLocation;
                    markingadjsdetail.Subitem = rowView[2].ToString().Replace(".", "");
                    markingadjsdetail.Qty = Convert.ToDecimal(rowView[19]) - Convert.ToDecimal(rowView[16]);
                    markingadjsdetail.Description = rowView[3].ToString();
                    markingadjsdetail.WarehouseID = tbWarehouse.Text;
                    markingadjsdetail.UOM = rowView[20].ToString();
                    markingadjsdetail.ReasonCode = "ADJUSTMENT";

                    markingadjsdetails.Add(markingadjsdetail);
                }
                markingadjs.Details = markingadjsdetails;
                ...
                else if(dv_filter.Count > 0)
                {
                    var authApi = new AuthApi(AcumaticaCred.AcumaticaSiteURL);
                    var configuration = LogIn(authApi, AcumaticaCred.AcumaticaSiteURL, AcumaticaCred.AcumaticaUser, AcumaticaCred.AcumaticaPassword, AcumaticaCred.AcumaticaTenant, AcumaticaCred.AcumaticaBranch, AcumaticaCred.AcumaticaLocale);
                    try
                    {
                        var markingadjsApi = new AdjustmentApi(configuration);
                        var responsemarkingadjs = markingadjsApi.PutEntity(markingadjs);
                        ReleaseAdjustment releasemarkingadjs = new ReleaseAdjustment((Adjustment)responsemarkingadjs);
                        markingadjsApi.InvokeAction(releasemarkingadjs);

                        referenceNbr = responsemarkingadjs.ReferenceNbr.ToString();
                        adjs.Text = referenceNbr;
                    }
                    catch (Exception ePut)
                    {
                        MessageBox.Show($"--Sync error! {ePut.Message}", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    ...
                    finally
                    {
                        ...
                        if (!syncError)
                        {
                            ...
                            saveDocument();
                            MessageBox.Show($"--Sync Complete Adjs!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Warning);
